...
IMAGE=${IMAGE:-localhost:5000/stream-bench}
OUTPUT_DIR="results"
CSV_FILE="$OUTPUT_DIR/stream_startup_results.csv"
...
extract_times() {
  ...
  ready=$(echo "$output" | grep -oP 'STREAM_READY_AT:\K\d+' || true)
  finished=$(echo "$output" | grep -oP 'STREAM_FINISHED_AT:\K\d+' || true)
...
{
  "metadata": {"name": "stream-pod", "namespace": "default", "attempt": 1, "uid": "stream-pod-uid"},
  "log_directory": "/tmp",
  "linux": {}
}
EOF
