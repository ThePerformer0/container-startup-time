...
IMAGE=${IMAGE:-localhost:5000/fio-bench}
OUTPUT_DIR="results"
CSV_FILE="$OUTPUT_DIR/fio_startup_results.csv"
...
extract_times() {
  ...
  ready=$(echo "$output" | grep -oP 'FIO_BENCHMARK_READY_AT:\K\d+' || true)
  finished=$(echo "$output" | grep -oP 'FIO_BENCHMARK_FINISHED_AT:\K\d+' || true)
