...
IMAGE=${IMAGE:-localhost:5000/unixbench-bench}
OUTPUT_DIR="results"
CSV_FILE="$OUTPUT_DIR/unixbench_startup_results.csv"
...
extract_times() {
  ...
  ready=$(echo "$output" | grep -oP 'UNIXBENCH_READY_AT:\K\d+' || true)
  finished=$(echo "$output" | grep -oP 'UNIXBENCH_FINISHED_AT:\K\d+' || true)
...
{
  "metadata": {"name": "unixbench-pod", "namespace": "default", "attempt": 1, "uid": "unixbench-pod-uid"},
  "log_directory": "/tmp",
  "linux": {}
}
EOF
...
{
  "metadata": {"name": "unixbench"},
  "image": {"image": "$IMAGE"},
  "args": [],
  "command": [],
  "working_dir": "/",
  "log_path": "unixbench.0.log",
  "stdin": false,
  "stdin_once": false,
  "tty": false,
  "linux": {"resources": {}}
